...
def cross_validation(X_train, y_train, print_list, subject):
    ...
    # Save AUROC
    fig_dir = os.path.join("Figures_CART_dt", "AUROC_"+subject+".png")
    plt.savefig(fig_dir)
    # plt.show()
...
def evaluate_model(X_train, X_test, y_train, y_test, print_list, subject):
    ...
    auroc = metrics.roc_auc_score(y_test, dt_CART.predict_proba(X_test)[:,1], average='macro')
    ...
    plot_confusion_matrix(dt_CART, X_test, y_test, display_labels=("Interictal", "Ictal"),
                          cmap=plt.cm.Blues, normalize="true")

    # Save Confusion Matrix
    fig_dir = os.path.join("Figures_CART_dt", "CM_"+subject+".png")
    plt.savefig(fig_dir)
    # plt.show()
...
if __name__ == '__main__':
    ...
    toprint = []
    toprint.append("CART DT Prediction Summary")
    toprint.append("\n")
    ...
    # Write file
    textfile = open(r"results_CART_dt.txt", "w")
    for elem in toprint:
        print(elem)
