function [corr_ftrs, names] = corr_features_t(data)
%
% Outputs correlation features among columns of given data in the time
% domain
%
% usage
% supply filtered eeg data to compute the correlation coefficients and 
% eigenvalues in the time domain
...
for i=1:size(corr_coefs, 1)
   for  j=i+1:size(corr_coefs, 2)
       ...
       names_coefs(1,idx) = sprintf('time_corr_coef%d',idx);
       idx = idx + 1;
   end
end
...
for i=1:length(eig_corr)
    ...
    names_eigs(1,i) = sprintf('time_corr_eig%d',i);
end
