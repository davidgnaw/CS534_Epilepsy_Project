...
# Description: Application of SVM and KNN to the features extracted from each patient of the
# publicly available dataset "American Epilepsy Society Seizure Prediction Challenge" and the
# "American Epilepsy Society Seizure Prediction Challenge" downloaded from kaggle
...
def import_features(file_name, print_list, task):
    ...
    if task == 'prediction':
        ...
        print_list.append("1. One-second eeg segments: {}".format(X.shape[0]))
...
def cross_validation(X_train, y_train, print_list, subject, task, model):
    ...
    if task == 'detection':
        if model == 'svm':
            fig_dir = os.path.join("F:/Users/user/Desktop/EMORY/Classes/Fall_2021/CS_534/Project/Detection/Figures_SVM", "AUROC_"+subject+".png")
        elif model == 'knn':
            fig_dir = os.path.join("F:/Users/user/Desktop/EMORY/Classes/Fall_2021/CS_534/Project/Detection/Figures_KNN", "AUROC_" + subject + ".png")
...
def evaluate_model(X_train, X_test, y_train, y_test, print_list, subject, task, model):

    # Train model
    if model == 'svm':
        model_eeg = svm.SVC(kernel="linear", probability=True, random_state=42)
        model_eeg.fit(X_train, y_train)
    elif model == 'knn':
        model_eeg = KNeighborsClassifier()
        model_eeg.fit(X_train, y_train)

    # Obtain predictions
    y_hat = model_eeg.predict(X_test)
    ...
    if model == 'svm':
        auroc = metrics.roc_auc_score(y_test, model_eeg.decision_function(X_test), average='macro')
    elif model == 'knn':
        auroc = metrics.roc_auc_score(y_test, model_eeg.predict_proba(X_test)[:,1], average='macro')
    ...
    plot_confusion_matrix(model_eeg, X_test, y_test, display_labels=("Interictal", "Ictal"), cmap=plt.cm.Blues,
                          normalize="true")
...
if __name__ == '__main__':
    ...
    if task == 'detection':
        if model == 'svm':
            toprint.append("SVM Detection Summary")
        elif model == 'knn':
            toprint.append("KNN Detection Summary")
        # Define parent directory
        parent_Dir = Path("F:/Users/user/Desktop/EMORY/Classes/Fall_2021/CS_534/Project/Detection/Extracted_Features/Train")
    elif task == 'prediction':
        if model == 'svm':
    ...
    for filename in files:
        ...
        X, y, feature_names, toprint = import_features(filename, toprint, task)
        ...
        toprint = cross_validation(X_train, y_train, toprint, subject, task, model)

        # Evaluation metrics and confusion matrix
        toprint = evaluate_model(X_train, X_test, y_train, y_test, toprint, subject, task, model)


    # Write file
    if task == 'detection':
        if model == 'svm':
            textfile = open(r"F:\Users\user\Desktop\EMORY\Classes\Fall_2021\CS_534\Project\Detection\Results_SVM.txt", "w")
        elif model == 'knn':
            textfile = open(r"F:\Users\user\Desktop\EMORY\Classes\Fall_2021\CS_534\Project\Detection\Results_KNN.txt", "w")
    elif task == 'prediction':
        if model == 'svm':
